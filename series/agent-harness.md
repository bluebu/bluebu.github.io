---
layout: default
title: Agent Harness 实践
permalink: "/series/agent-harness/"
---

<h1>Agent Harness 实践</h1>
<p class="muted">用架构约束 AI，而不是用 prompt 求 AI。</p>

{% if jekyll.environment == "production" %}
<p class="muted">部分内容需要脱敏，暂时关闭浏览。</p>
{% else %}
<ul class="articles">
  <li>
    <h2><a href="/posts/agent-harness-pattern">一、让 AI 写代码不翻车</a></h2>
    <div class="content">背景、问题、Agent Harness 四层架构概述</div>
  </li>
  <li>
    <h2><a href="/posts/agent-harness-tdd">二、TDD 红绿灯</a></h2>
    <div class="content">strict / fix 模式，单语言闭环，不跳红、测试只读、禁删用例</div>
  </li>
  <li>
    <h2><a href="/posts/agent-harness-ddr">三、DDR 自我审计</a></h2>
    <div class="content">Detect → Diagnose → Remediate，plan 文件为什么是关键</div>
  </li>
  <li>
    <h2><a href="/posts/agent-harness-efficiency">四、效率守卫</a></h2>
    <div class="content">六种反模式识别，熔断机制，阈值迭代</div>
  </li>
  <li>
    <h2><a href="/posts/agent-harness-subagent">五、Subagent 分工设计</a></h2>
    <div class="content">拆分原则，文件权限表，Agent 协作链路</div>
  </li>
  <li>
    <h2><a href="/posts/agent-harness-skill">六、Skill 编排设计</a></h2>
    <div class="content">Intake 协议，Review 三种模式，端到端工作流</div>
  </li>
  <li>
    <h2><a href="/posts/agent-harness-bootstrap">七、从零开始搭建</a></h2>
    <div class="content">落地节奏，常见误区，一个月从零到成型</div>
  </li>
  <li>
    <h2><a href="/posts/agent-harness-cheatsheet">终篇、速查手册</a></h2>
    <div class="content">纯干货，模板 + 结构，拿去抄</div>
  </li>
</ul>
{% endif %}
